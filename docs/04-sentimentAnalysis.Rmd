...
```{r bts_news_comment, R.options=list(tibble.width = 50), eval=F}
# 전처리
library(stringr)
library(textclean)
news_comment <- raw_news_comment %>%
...
```{r ref.label="bts_news_comment", echo=F, R.options=list(tibble.width = 50)}
...
```{r, q2_score_comment,R.options=list(tibble.width = 50), eval=FALSE}
# 댓글별로 단어의 감정 점수 합산
score_comment <- word_comment %>%
  group_by(id, reply) %>%
  summarise(score = sum(polarity)) %>%
...
```{r, ref.label="q2_score_comment", R.options=list(tibble.width = 50), echo=FALSE}
```
