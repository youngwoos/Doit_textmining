...
- `tada`에 `word_sympathy` 결합하기
  - `tada`: 댓글 원문
  - `word_sympathy`: `sympathy`(공감 여부), `diff`(공감, 비공감 수 차이)가 들어 있음
      - 각 행이 댓글별 명사이므로 여러 댓글이 중복되어  있음
      - 댓글별로 한 행씩만 남기고 주요 변수를 추출한 다음 결합
...
- 공감, 비공감 댓글 각각 추출
  - 공감 댓글은 공감 수가 높은 순으로, 비공감 댓글은 비공감 수가 높은 순으로 정렬
  - 출력하면 주요 댓글 중심으로 살펴볼 수 있음
