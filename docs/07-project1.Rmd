...
knitr::opts_chunk$set(cache = T, 
                      warning = F, 
                      message = T,  # True for crayon setup
                      dpi = 300, 
                      fig.height = 4)
...
`r fontawesome("lightbulb")` 여러 단어를 함께 언급한 댓글도 있기 때문에 `bind_category`에는 댓글 내용은 같지만 카테고리는 다른 행이 있음(ex: `"택시업계"`와 `"정부"`를 함께 언급한 댓글)
...
```{r out_tfidf_category, results='hide'}
# 주요 단어 추출, 불용어 확인
tfidf_category %>%
  group_by(category) %>%
...
---
```{r ref.label="out_tfidf_category", echo=F}
```
---
...
---
```{r ref.label="p_word_cors", echo=F}
```
---
...
- **타다**
  ...
  - `"서비스"`, `"편하다"`, `"좋다"`, `"많다"`: 타다의 장점을 표현한 단어와 관련성이 큽니다.
...
- **정부**
  ...
  - `"기업"`, `"자유"`, `"시장"`, `"경제"`, `"규제"`, `"막다"`: 정부가 산업 발전을 막는다는 비판을 할 때 사용하는 단어와 관련성이 크다
